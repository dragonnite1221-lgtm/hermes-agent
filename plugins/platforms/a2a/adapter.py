...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _advertised_skills(self, agent: Optional[dict] = None) -> list[dict]:
        """Agent Card skills from ``advertised_toolsets`` (the agent's declared capabilities),
        enriched with live tool names from the registry; an unrestricted agent advertises whatever
        the registry knows. Static fallback without a registry."""
        configured = (agent or {}).get("advertised_toolsets") if agent else self._advertised_toolsets
        names = [str(n) for n in (configured or [])]
        try:
            from tools.registry import registry as tool_registry
            registered = set(tool_registry.get_registered_toolset_names())
            # A configured capability stays on the card even when no toolset of that name is
            # registered locally: served agents declare capabilities (possibly served by a remote
            # profile), so the registry may only enrich tags -- it must never drop a declared one.
            mapping = {n: (tool_registry.get_tool_names_for_toolset(n) if n in registered else [])
                       for n in (names or sorted(registered))}
            if mapping:
                return protocol.skills_from_toolsets(mapping)
        except Exception:
            logger.debug("A2A: tool registry unavailable for Agent Card", exc_info=True)
        return protocol.skills_from_toolsets(names)
    ...
    def _task_scope(self, agent: Optional[dict], peer: str) -> tuple[str, str, str]:
        agent_slug, tenant = self._scope_for_agent(agent)
        return agent_slug, tenant, str(peer or "")
    ...
    def _rpc_tasks_subscribe(
        ...
        self._sse_headers(handler)
        try:
            if (fut := self.tasks.watch(task_id, *self._task_scope(agent, peer))) is None:
    ...
    def _find_task(self, req_id: Any, params: dict, agent: Optional[dict], peer: str = "") -> tuple[str, Optional[dict], Optional[dict]]:
        ...
        rec = self.tasks.get(task_id, *self._task_scope(agent, peer))
    ...
    def _rpc_tasks_list(self, req_id: Any, params: dict, agent: Optional[dict] = None, peer: str = "") -> dict:
        ...
        recs, next_offset, total = self.tasks.list(
            ...
            page_size=page_size, offset=max(0, offset), agent_slug=agent_slug, tenant=tenant, peer=peer, with_total=True)
    ...
    def _rpc_tasks_cancel(self, req_id: Any, params: dict, agent: Optional[dict] = None, peer: str = "") -> dict:
        ...
        rec = self.tasks.get(task_id, *self._task_scope(agent, peer)) or rec
    ...
    def _register_inline_push(
        ...
        if url:
            self.tasks.set_push_config(task_id, str(url), *self._task_scope(agent, peer))
    ...
    def _rpc_push_config_create(
        ...
        stored = self.tasks.set_push_config(task_id, url, *self._task_scope(agent, peer))
    ...
    def _push_config_op(self, req_id: Any, params: dict, agent: Optional[dict], peer: str, op, render) -> dict:
        """Shared get/list/delete: ``op(task_id, config_id, slug, tenant, peer)`` falsy => not found."""
        task_id = str(params.get("taskId") or "")
        if not task_id:
            return _err(req_id, protocol.ERR_INVALID_PARAMS, "taskId required")
        found = op(task_id, str(params.get("id") or params.get("configId") or ""), *self._task_scope(agent, peer))
    ...
    def _send_push_notification(self, task_id: str, context_id: str, reply: str, state: str) -> None:
        ...
        try:
            ...
            status = security.post_safe_callback(
                callback_url,
                data,
                headers,
                timeout=10,
