...
def _patch_update_deps(monkeypatch, tmp_path, run_side_effect):
    ...
    # .get() on it. Returning None here (as this mock used to, twice)
    # raises AttributeError, which the caller's except-block turns into an
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **k: {"matched": [], "killed": [], "failed": [], "unrecovered": []},
    )
