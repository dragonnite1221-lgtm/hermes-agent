...
class TestSystemUnitHermesHome:
    ...
    def test_managed_node_makes_system_unit_independent_of_callers_path(
        ...
        # Scoped to "node" only: a blanket `lambda name: ...` here also answers the unrelated
        # powershell.exe/cmd.exe/explorer.exe/wsl.exe lookups _build_wsl_interop_paths() makes when
        # running under WSL, corrupting them with a fake node path and producing a spurious diff
        # between root_unit/user_unit that has nothing to do with the node-independence behavior
        # under test.
        monkeypatch.setattr(
            gateway_cli.shutil, "which", lambda name: "/root/bin/node" if name == "node" else None
        )
        root_unit = gateway_cli.generate_systemd_unit(system=True, run_as_user="alice")

        monkeypatch.setattr(
            gateway_cli.shutil, "which", lambda name: "/home/alice/.local/bin/node" if name == "node" else None
        )
