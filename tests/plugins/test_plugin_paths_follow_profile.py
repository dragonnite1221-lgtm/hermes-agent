...
def _a2a_conversation(home):
    ...
    return protocol._conv_dir() / f"{protocol._safe_name('peer-x')}.jsonl"
...
_RESOLVERS = {
    "a2a": _a2a_conversation,
    "photon": _photon_auth,
    "mem0-qdrant": _mem0_qdrant,
...
def test_plugin_path_follows_profile_override(name, tmp_path, monkeypatch):
    ...
    assert resolved.startswith(str(profile)), (
        f"{name} resolved {resolved!r} outside the active profile"
    )
