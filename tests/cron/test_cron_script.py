...
class TestRunJobScript:
    ...
    def test_global_timeout_above_per_job_limit_is_preserved(
        ...
        assert scheduler_script._get_script_timeout() == 30 * 24 * 60 * 60
    ...
    def test_explicit_timeout_overrides_global_config(self, cron_env, monkeypatch):
        ...
        monkeypatch.setattr(scheduler_script.subprocess, "Popen", NeverFinishes)
        monotonic = iter((100.0, 103.0))
        monkeypatch.setattr(scheduler_script.time, "monotonic", lambda: next(monotonic))
        monkeypatch.setattr(scheduler_script, "_get_script_timeout", lambda: 600)
        monkeypatch.setattr(scheduler_script, "_terminate_cron_script_tree", lambda _proc: None)
        monkeypatch.setattr(scheduler_script, "_drain_script_pipes", lambda _proc: None)
    ...
    def test_oversized_runtime_timeout_falls_back_before_spawn(
        ...
        monkeypatch.setattr(scheduler_script, "_get_script_timeout", lambda: 30)
        monkeypatch.setattr(scheduler_script.time, "monotonic", lambda: 100.0)
        ...
        monkeypatch.setattr(scheduler_script.subprocess, "Popen", CapturedProcess)
    ...
    def test_deadline_failure_cannot_orphan_a_child(self, cron_env, monkeypatch):
        ...
        success, output = scheduler_script._run_job_script("noop.py", timeout_seconds=30)
