...
from tools.cronjob_job_args import (
    ...
    _canonical_skills,
    _clean_str_list,
    _format_job as _format_job_base,
...
def claim_job_for_fire(
    ...
    *,
    ...
    if return_job:
        if claim_ttl_seconds is None:
            return claim_fire_with_execution(
                job_id, source="direct", force=force, manual=manual,
            )
        return claim_fire_with_execution(
            job_id,
            source="direct",
    ...
    if claim_ttl_seconds is None:
        return _claim_job_for_fire(job_id, force=force, manual=manual)
...
def _format_job(job: Dict[str, Any]) -> Dict[str, Any]:
    """Wrap upstream's extracted ``_format_job`` (tools/cronjob_job_args.py), re-adding the
    fork-only ``retry_interrupted``/``script_timeout_seconds`` fields it doesn't know about."""
    result = _format_job_base(job)
    result["retry_interrupted"] = job.get("retry_interrupted") is True
...
def _execute_job_now(job: Dict[str, Any], extra_prompt: Optional[str] = None) -> Dict[str, Any]:
    """Run a job now, outside the scheduler tick.

    Registers the in-process running slot BEFORE the durable fire claim: claiming in the
    inverse order races the ticker — a rejected manual run could otherwise strand both its
    ledger-first execution and its fire claim (neither released nor handed off to a run).
    ...
    # Acquire the in-process slot before the durable fire claim. Claiming in
    # the inverse order races the ticker: a rejected manual run can otherwise
    # strand both its ledger-first execution and its fire claim.
    if not try_register_running_job(job_id):
        return {"claimed": False, "success": False, "error": _ALREADY_RUNNING_ERROR}
    try:
        # At-most-once claim: bail without running if a tick/other fire owns it. ``manual=True``
        # marks this as an off-tick run-now so the claim doesn't stamp next_run_at as the
        # consumed occurrence (that still-pending slot would otherwise be skipped when it arrives).
...
def _run_claimed_job(
    ...
    *,
    ...
    to a worker). ``already_registered`` skips re-registering the in-process running slot
    when the caller already holds it (e.g. the background-dispatch path, which must keep the
    slot for the whole async window). Returns {"claimed": True, "success": bool, "error": ...}."""
    job_id = job["id"]
    ...
    try:
        ...
        # Matrix/aiohttp (a standalone asyncio.run() loop breaks them). Manual runs invoked
        # from a gateway agent execute outside the scheduler ticker, but they still share the
        # process with the live platform adapters. Calling those clients from run_one_job's
        # standalone asyncio.run() loop raises errors like "Timeout context manager should be
        # used inside a task" and can break encrypted Matrix delivery (#61495 — salvaged from
        # #63586 by @Fly-onlyone).
        ...
        try:
            ...
            # A manual `run` executes synchronously on the caller's thread, and a cron job is
            # itself a full agent run that routinely takes minutes with no tool activity on the
            # calling turn — without a heartbeat the gateway inactivity watchdog would conclude
            # the agent is hung and kill the parent turn (#76502); ``_run_heartbeat`` covers it.
        ...
        if _registered:
            ...
            with contextlib.suppress(Exception):
                release_running_job(job_id)
        if not _run_started:
            # No owner-bearing snapshot reached run_one_job, so this caller cannot prove it
            # acquired the fire. Recording a job failure here could consume an owed
            # interrupted-retry marker for work that never started — abort the ledger-first
            # execution/claim instead.
            try:
                ...
                logger.exception(
                    "Failed to restore cron job %s after manual pre-run abort", job_id)
        else:
            with contextlib.suppress(Exception):
...
def _try_dispatch_background_run(
    ...
    # Hold the authoritative in-process slot across the durable claim and the
    # background handoff. A read-only running-set precheck leaves a race where
    # the ticker registers after the check but before this path claims.
    if not try_register_running_job(job_id):
        return {"claimed": False, "success": False, "error": _ALREADY_RUNNING_ERROR}
    try:
        # Same snapshot claim as _execute_job_now: carry the owner-bearing record into the
        # run so terminal writes stay fenced by this owner. ``manual=True`` marks this as an
        # off-tick run-now so the claim doesn't stamp next_run_at as the consumed occurrence.
        claimed_job = claim_job_for_fire(job_id, manual=True, return_job=True)
        if not isinstance(claimed_job, dict):
            release_running_job(job_id)
            refreshed = get_job(job_id)
...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            # CLI/operator-only: deliberately absent from the model schema and dispatch because
            # it opts into at-least-once side-effect semantics.
            retry_interrupted=(
                a["retry_interrupted"] if a["retry_interrupted"] is not None else False
            ),
