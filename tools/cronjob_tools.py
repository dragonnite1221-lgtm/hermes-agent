...
def claim_job_for_fire(
    ...
    *,
    ...
    ``manual`` is forwarded only when truthy, so callers that predate this parameter (and any
    test asserting the exact call signature) see the identical underlying call as before it
    existed.
    """
    ...
    if claim_ttl_seconds is not None:
        kwargs["claim_ttl_seconds"] = claim_ttl_seconds
    if return_job:
        return claim_fire_with_execution(job_id, source="direct", **kwargs)
    return _claim_job_for_fire(job_id, **kwargs)
...
def _claim_for_manual_run(job_id: str, log_label: str):
    ...
    ``(None, error_dict)`` in the ``_execute_job_now`` shape.

    Registers the in-process running-job slot in the scheduler's shared running set BEFORE the
    durable fire claim. Claiming in the inverse order races the ticker: a rejected manual run
    could otherwise strand both its ledger-first execution and its fire claim. A lost claim is
    labelled precisely — claim_job_for_fire also returns False for paused/disabled/missing jobs,
    not just in-flight ones.
    """
    ...
    try:
        ...
        # claim_job_for_fire returns False for paused/disabled/missing jobs too — don't
        # mislabel those as "already being fired" (#60703): that message sends the user
        # chasing a phantom in-flight run when the job simply isn't runnable.
        ...
        release_running_job(job_id)
        logger.error("Failed to set up cron job %s for %s: %s", job_id, log_label, e)
        ...
        # No owner-bearing snapshot was returned, so this caller cannot prove it acquired
        # the fire. claim_job_for_fire is atomic (it raises without committing a claim, or
        # returns a dict) — recording a job failure here could wrongly consume an owed
        # interrupted-retry marker for work that never started.
        return None, {"claimed": False, "success": False, "error": str(e)}
...
def _execute_job_now(job: Dict[str, Any], extra_prompt: Optional[str] = None) -> Dict[str, Any]:
    ...
    return err if err is not None else _run_claimed_job(
        claimed_job, extra_prompt=extra_prompt, already_registered=True)
...
def _run_claimed_job(
    ...
    *,
    ...
    to a worker). ``already_registered=True`` skips the in-process running-job registration
    when the caller (``_claim_for_manual_run``) already took it — that path registers BEFORE
    the durable fire claim to avoid stranding it, so redoing the check here would be a no-op
    at best; a standalone caller (tests, direct callers) still gets the authoritative dedupe.
    Returns {"claimed": True, "success": bool, "error": ...}."""
    job_id = job["id"]
    ...
    try:
        ...
        # In-flight dedupe (idea from #53395 by @izumi0uu): the fire claim's TTL is routinely
        # outlived by real jobs, so it alone cannot stop a manual run from double-firing a job
        # the ticker (or another manual run) is still executing. Register in the scheduler's
        # shared running set — the same guard _submit_with_guard uses — which also makes this
        # run visible to the gateway shutdown drain and mark_running_jobs_interrupted.
        if not _registered and not try_register_running_job(job_id):
        ...
        if not processed and not run_error:
            run_error = (
        ...
        if _registered:
            ...
            with contextlib.suppress(Exception):
                ...
                _release(job_id)
        if not _run_started:
            # run_one_job never started: nothing has written a terminal status for this
            # fire, so restore the claim/ledger via the CAS-fencing abort path rather than
            # a normal mark_job_run (which could wrongly consume an owed interrupted-retry
            # marker for work that never executed).
            try:
...
def _try_dispatch_background_run(
    ...
    # Synchronous claim (same semantics as _execute_job_now, which also registers the
    # in-process running-job slot BEFORE the durable fire claim to avoid stranding it).
    claimed_job, err = _claim_for_manual_run(job_id, "background run")
    if err is not None:
        # Same shape as _execute_job_now, plus dispatched=False: this code path was
        # entered specifically to attempt a background dispatch, so the caller's
        # response formatting expects the key regardless of whether the claim was
        # merely lost or setup raised.
    ...
    try:
        ...
        result = _run_claimed_job(claimed_job, extra_prompt=extra_prompt, already_registered=True)
    ...
    def _runner() -> Dict[str, Any]:
        res = _run_claimed_job(claimed_job, extra_prompt=extra_prompt, already_registered=True)
...
def _action_create(a: Dict[str, Any]) -> str:
    ...
    try:
        job = create_job_with_scheduler_registration(
            ...
            # CLI/operator-only: deliberately absent from the model schema and dispatch because it
            # opts into at-least-once side-effect semantics an agent must not grant itself.
            retry_interrupted=a["retry_interrupted"] if a["retry_interrupted"] is not None else False,
            # CLI/operator-only: deliberately absent from the model schema so an agent cannot
            # widen its own execution budget.
...
def _update_core_fields(job: Dict[str, Any], a: Dict[str, Any], updates: Dict[str, Any]) -> Optional[str]:
    ...
    if a["retry_interrupted"] is not None:
        updates["retry_interrupted"] = a["retry_interrupted"]
