...
def _restart_managed_dashboard_service(
    reason: str, unit: str = _DASHBOARD_SYSTEMD_UNIT
) -> dict[str, list[str]] | None:
    """Restart a systemd-managed dashboard instead of raw-killing its PID.

    Returns ``None`` when no managed unit was found. Once a unit is found, returns a truthy
    result even when its restart fails so the caller never falls back to ``os.kill``: systemd
    treats a direct SIGTERM of the service's main PID as a clean stop, so ``Restart=on-failure``
    will not bring the dashboard back. ``restarted_services`` records only a verified successful
    restart, allowing post-update reconciliation to distinguish a handled failure from a service
    ...
    for command in commands:
        ...
        if result.returncode == 0:
            ...
            return {"restarted_services": [unit], "failed_services": []}
    ...
    return {"restarted_services": [], "failed_services": [unit]}
