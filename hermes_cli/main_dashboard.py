...
def _restart_managed_dashboard_service(
    ...
    ``False`` when no such unit exists (caller falls through to raw-kill/respawn).
    Otherwise a unit was found and handled — which deliberately stops the caller's
    ``os.kill`` fallback (systemd treats a direct SIGTERM as a clean stop, so
    ``Restart=on-failure`` won't bring it back) — and a dict reports which outcome:
    ``{"restarted_services": [unit]}`` on success, ``{"failed_services": [unit]}``
    on a failed restart, so update-plan reconciliation doesn't count a failed
