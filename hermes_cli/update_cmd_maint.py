...
def _finish_dashboard_update_cleanup(
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    Returns the ``_kill_stale_dashboard_processes`` outcome so the caller can fold it into
    the update's runtime bookkeeping via ``_fold_dashboard_cleanup_runtime_bookkeeping``
    (a PID stopped here but left ``unrecovered`` must not read as a clean stop during plan
    reconciliation). The untouched-dashboard early return yields an empty-but-shaped dict
    rather than ``None`` so callers never need a null guard.
