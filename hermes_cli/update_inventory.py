...
# Supervisors whose gateways are actually owned by a service manager, so a restarted unit
# name is evidence about THAT process.
_SERVICE_SUPERVISORS = {"systemd", "launchd", "service", "windows-service"}


def _restarted_service_claims_gateway(r: RuntimeRecord, names: set) -> bool:
    """Whether a restarted *service* name may account for this gateway runtime.

    A restarted unit only speaks for the process that unit supervises. A manually launched
    gateway that merely shares the profile is a different process: crediting it with the
    unit's restart hides exactly the silent miss this phase exists to catch (#91277). The
    plan's own recorded unit is authoritative when the inventory captured one.
    """
    planned = (getattr(r, "detail", None) or {}).get("service")
    if planned:
        planned_name = str(planned).removesuffix(".service").rsplit("/", 1)[-1]
        if any(str(n).removesuffix(".service").rsplit("/", 1)[-1] == planned_name for n in names):
            return True
    if r.supervisor not in _SERVICE_SUPERVISORS:
        return False
    return _gateway_named_in(r, names)
...
def match_runtime_outcomes(
    ...
    planned runtime; outcome is ``restarted``, ``externally-supervised`` (a Desktop-owned
    serve/dashboard backend intentionally preserved by the update), ``stopped``, ``failed`` or
    ``unaccounted`` (no bookkeeping mentions it — the blind-spot tripwire). A restarted service
    name only accounts for a runtime that service actually supervises: a manual gateway sharing
    the profile stays unaccounted. Never raises. Serve/dashboard runtimes are
    ...
    try:
        ...
        def _outcome(r: RuntimeRecord) -> str:
            ...
            if _gateway_named_in(r, failed_set):
                return "failed"
            return "restarted" if _restarted_service_claims_gateway(r, restarted_set) else "unaccounted"
