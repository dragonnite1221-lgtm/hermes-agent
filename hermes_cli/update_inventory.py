...
# Supervisors whose gateway is actually owned by a service manager, so a restarted-unit
# name is real evidence about THAT process rather than a coincidental profile-name
# collision with an unrelated unit ("manual"/"manual-serve" gateways restart through
# no service manager at all, so a same-named unit restarting proves nothing about them).
        ...
        def _outcome(r: RuntimeRecord) -> str:
            ...
            if r.kind in _SERVE_KINDS:
                if r.restart_via == "desktop" and stale_serves is None:
                    # The Desktop app owns this backend's entire lifecycle (start, stop, respawn)
                    # outside the update's own restart bookkeeping — nothing here to confirm
                    # against restarted_set/killed, and there never will be. When the survivor
                    # probe DID run (stale_serves is not None) and shows this exact PID is still
                    # the pre-update incarnation, that is a confirmed unsafe survivor: let the
                    # probe override this default instead of masking it as "safely supervised".
            ...
            if killed_here:
                return "stopped"
            # A direct failed_units match is explicit, strong evidence and always wins,
            # regardless of supervisor. A restarted_services match is a weaker signal (a
            # generic "these units restarted" list) that only counts when this runtime's
            # supervisor is actually one of those services — otherwise a manual/desktop
            # gateway could falsely claim "restarted" from an unrelated unit sharing its
            # profile name, silently defeating the unaccounted-runtime tripwire.
