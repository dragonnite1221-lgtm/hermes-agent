...
def _job_rows(job: Dict[str, Any]) -> List[tuple[str, str]]:
    ...
    optional = [
        ...
        ("Timeout", f"{job['script_timeout_seconds']}s (job override)"
         if job.get("script_timeout_seconds") else ""),
        ...
        ("Restart", "retry interrupted run (at-least-once)"
         if job.get("retry_interrupted") else ""),
...
_JOB_DETAIL_LINES = (
    ...
    ("retry_interrupted", "  Restart: retry interrupted run (at-least-once)"),
