    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    if restart_managed:
        ...
        if managed_restart:
            ...
            _dash_unit = getattr(
                _dash, "_DASHBOARD_SYSTEMD_UNIT", "hermes-dashboard.service"
            )
    ...
    if killed and restart_managed:
        ...
        restarted_services += killed_restarted
        failed_services += killed_failed
    else:
    ...
    return {
        ...
        # restarted. Without this, an inventoried custom serve/dashboard service is reported
        # as untouched even after its verified systemd restart.
        "restarted_services": list(restarted_services),
        # Keep service restart failures separate from raw process-stop errors. The update
        # command folds both into its fail-closed completion state.
...
def _restart_killed_backends(
    ...
    logs/dashboard-restart.log; one per profile, no ``--port 0``). Returns
    ``(unrecovered_pids, restarted_services, failed_services)`` — the service lists feed the update
    plan's reconciliation with the exact owning units this cleanup restarted, so an inventoried
    custom serve/dashboard service is never reported as untouched after its verified restart."""
