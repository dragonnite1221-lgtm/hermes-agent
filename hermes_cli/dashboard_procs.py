...
def _kill_stale_dashboard_processes(
    ...
    *already_restarted_units* names units (no ``.service`` suffix) the caller already restarted directly —
    ...
    if restart_managed:
        managed_restart = _dash._restart_managed_dashboard_service(reason)
        if managed_restart:
            ...
            if isinstance(managed_restart, dict):
                # Some restart backends report per-service detail; fold it into the
                # cleanup result so update-plan reconciliation sees the exact units.
    ...
    # Only meaningful on Linux, and only when the caller asked for restarts (the `hermes update`
    # path) — `--stop` must stay a stop, not a restart.
    ...
    if restart_managed and sys.platform != "win32":
        ...
        # The process table is scanned after the managed restart, so it may contain the unit's
        # newly-created PID; the ``already_restarted_units`` filter just below (which now
        # includes the dashboard unit, see above) excludes that exact owner while preserving
        # every co-located manual/custom unit.
    ...
    if killed and restart_managed:
        unrecovered, killed_restarted, killed_failed = _restart_killed_backends(
            killed, pid_service, pid_cgroup, pid_cmdline, pid_home)
        restarted_services.extend(killed_restarted)
        failed_services.extend(killed_failed)
    else:
        unrecovered = list(killed)
        if killed:
            print("  Restart the dashboard when you're ready:\n    hermes dashboard --port <port>")
    return {
        "matched": list(pids), "killed": list(killed), "failed": list(failed),
        "unrecovered": list(unrecovered),
        # Feed the update plan's reconciliation with the exact owning units this cleanup
        # restarted. Without this, an inventoried custom serve/dashboard service is reported as
        # untouched even after its verified systemd restart.
        "restarted_services": restarted_services,
        # Keep service restart failures separate from raw process-stop errors. The update command
        # folds both into its fail-closed completion state.
...
def _restart_killed_backends(
    ...
    logs/dashboard-restart.log; one per profile, no ``--port 0``).

    Returns ``(unrecovered pids, restarted service names, failed service names)``.
    """
    ...
    return unrecovered, restarted_services, failed_services
