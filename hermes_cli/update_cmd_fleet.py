...
def _verify_fleet_after_update(restart, *, _pre_update_plan, _windows_gateway_resume, node_failures, update_complete):
    ...
    """
    from hermes_cli.update_cmd import (
        _finish_dashboard_update_cleanup, _fold_dashboard_cleanup_runtime_bookkeeping, _m,
    ...
    # Fold this later cleanup phase into the SAME bookkeeping the fleet probe and the
    # plan-vs-execution reconciliation below read, so a backend we actually stopped or
    # restarted here is not reported as untouched — and a PID stopped but left
    # ``unrecovered`` is not miscounted as a clean stop. Must run before
    # ``fleet_probe_signals()``/``match_runtime_outcomes`` consume killed_pids.
    try:
        if _fold_dashboard_cleanup_runtime_bookkeeping(
            _dashboard_cleanup or {},
        ...
        logger.debug(
            "Could not fold dashboard cleanup into runtime bookkeeping: %s", _cleanup_bookkeeping_exc
        )
        # Bookkeeping is part of the update's proof that every runtime was restored. If it
        # cannot be interpreted, fail closed rather than issuing a success receipt over
