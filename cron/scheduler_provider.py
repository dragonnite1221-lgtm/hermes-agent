...
def abort_fire_claim_execution(
    ...
    *,
    ...
    # above succeeds. If it does not, release the still-active row to the
    # periodic recovery pass now: terminalizing it here would strand the
    # still-owned fire claim. Deferred to this point (rather than called
    # eagerly before the rollback attempt) so a *successful* rollback falls
    # through to finish_execution below with the row's original owner
    # identity intact — release_execution_for_recovery reassigns process_id/
    # pid to a released marker, which would otherwise fail finish_execution's
    # same-owner CAS.
...
class CronScheduler(ABC):
    ...
    def fire_due(
        ...
        at-most-once) then shared ``run_one_job``. True if THIS caller claimed the occurrence
        and the workload started, even if the workload itself failed; False if the claim was
        lost, the job vanished, or pre-run setup aborted while preserving the occurrence for
        retry. ``manual`` marks an off-tick run-now (dashboard trigger): the claim must not
        stamp ``next_run_at`` as the occurrence, or that slot is skipped when it arrives.
        Webhook and misfire fires run the slot that is due and keep the stamp."""
    ...
    def claim_fire(self, job_id: str, *, force: bool = False, manual: bool = False) -> dict | None:
        ...
        """
        claimed_job = claim_fire_with_execution(
            job_id, source=self.name, force=force, manual=manual
        )
        if claimed_job is not None:
            from cron.executions import set_execution_occurrence

            # Bind the exact claimed occurrence before handing the snapshot off — the
            # execution record was necessarily created before the instant was known.
            set_execution_occurrence(
                claimed_job["execution_id"], claimed_job.get("_scheduled_instant")
            )
        return claimed_job
