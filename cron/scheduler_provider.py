...
def claim_fire_with_execution(
    ...
    *,
    ...
    ``manual`` marks an off-tick run-now (dashboard trigger): forwarded to the
    jobs-store claim so it does not stamp ``next_run_at`` as the consumed
    occurrence, or that still-pending slot would be skipped when it arrives.
    """
    from cron.executions import (
        create_execution,
        discard_unacquired_execution,
    ...
    try:
        ...
        logger.warning(
            "Job '%s': failed to bind execution %s to its scheduled occurrence",
            job_id,
            execution["id"],
            exc_info=True,
        )
...
class CronScheduler(ABC):
    ...
    def claim_fire(self, job_id: str, *, force: bool = False, manual: bool = False) -> dict | None:
        ...
        ``fire_claimed`` in tracked background work. ``manual`` marks an
        off-tick run-now (dashboard trigger): see ``claim_fire_with_execution``.
        """
        return claim_fire_with_execution(job_id, source=self.name, force=force, manual=manual)
