...
def _timeout_from_env_or_config(
    env_var: Optional[str], config_key: str, parse: Callable[[Any], Any], label: str):
    ...
    ``env_var=None`` means this timeout has NO env surface: config.yaml is its sole public
    override. Do not hand a knob an env var it never documented — an undocumented env var that
    silently outranks config is a configuration-integrity hole, not a convenience."""
...
def _get_media_send_timeout() -> int:
    """Per-attachment media-send timeout: ``cron.media_send_timeout_seconds``, then 300s
    (long TTS audio can exceed a 30s window).

    Config-only by design (0fc082bbb3): unlike its sibling timeouts this knob has no documented
    env surface, so HERMES_CRON_MEDIA_SEND_TIMEOUT must NOT be honoured — an undocumented env
    var silently outranking config.yaml is exactly the override the fork removed."""
    resolved = _timeout_from_env_or_config(
        None, "media_send_timeout_seconds", _positive_int,
...
def _run_job_script(
    ...
    try:
        ...
        # Compute the deadline BEFORE spawning: if time.monotonic() itself raises (clock
        # overflow), there must be no child left to orphan.
