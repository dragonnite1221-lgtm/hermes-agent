...
def _get_media_send_timeout() -> int:
    """Per-attachment media-send timeout from ``cron.media_send_timeout_seconds``, else 300s
    (long TTS audio can exceed a 30s window). ``HERMES_CRON_MEDIA_SEND_TIMEOUT`` is NOT read here
    — unlike the script timeout, this setting has no documented env override, and honoring an
    undocumented/stray env var would silently defeat an explicit config value."""
    try:
        cfg = _sched.load_config() or {}
        cron_cfg = cfg.get("cron", {}) if isinstance(cfg, dict) else {}
        configured = cron_cfg.get("media_send_timeout_seconds")
        if configured is not None:
            timeout = _positive_int(configured)
            if timeout is not None:
                return timeout
    except Exception as exc:
...
def _run_job_script(
    ...
    instead of the scripts-dir parent. See #69396. timeout_seconds: Optional positive per-job timeout.
    None inherits the global ``cron.script_timeout_seconds`` setting.
    """
    ...
    if (
        ...
        if timeout_seconds is not None:
            logger.warning(
                "Invalid per-job script_timeout_seconds=%r; using config/default",
                timeout_seconds,
            )
    ...
    try:
        ...
        # Establish the wall-clock deadline BEFORE starting the child: even malformed runtime
        # state (e.g. a clock error) then cannot leave a newly spawned process orphaned.
