...
from cron.jobs import (
    ...
    clear_run_claim, finalize_fire_claim_setup, get_due_jobs, heartbeat_fire_claim,
    heartbeat_run_claim, mark_job_run, save_job_output, use_cron_store)
from cron.executions import (
    _TERMINAL_STATES, create_execution, discard_unacquired_execution, finish_execution,
    get_execution, heartbeat_execution, mark_execution_handoff_pending, mark_execution_running,
    mark_fire_claim_acquired, recover_interrupted_executions, remember_execution_recovery_intent)
...
def _run_with_fire_claim_heartbeat(job: dict, run) -> bool:
    ...
    def _abort_unstarted(error: str) -> None:
        ...
        Supersedes the plain ``finish_execution`` close: an attempt that never
        reached the workload must give its occurrence back so the
        interrupted-retry recovery policy can re-fire it, rather than burning
        the slot on a run that produced no side effect.
        """
    ...
    try:
        ...
        _abort_unstarted("Fire claim ownership could not be validated before execution started.")
        return False
    ...
    if heartbeat_thread is None:
        _abort_unstarted("Fire claim heartbeat could not be started; execution was not run.")
        return False
...
def run_one_job(
    ...
    claim (callers use the store CAS) but keeps it alive. True if the workload started and the
    attempt was processed (a job failure is recorded via ``mark_job_run``); False when pre-run
    setup aborts (the occurrence is rolled back for interrupted-retry recovery) or processing
    raised. ``cancel_event``: optional transport-level cancel (dashboard drain), OR-combined with
    the fire-claim heartbeat's lost-ownership event so either stops the run cooperatively."""
    ...
    try:
        ...
        try:
            ...
            if not external_owner and mark_execution_running(execution_id) is None:
                # Another owner already won the claimed->running CAS -- this
                # is a normal concurrent-claim outcome, not a failure needing
                # the abort/interrupted-retry recovery machinery below. Bail
                # out cleanly with no side effects.
            ...
            if _scope_token is not None:
                reset_secret_scope(_scope_token)
                # The outer finally also resets this scope on the way out;
                # clear the reference so it does not try to reuse an
                # already-consumed context token (#interrupted-retry merge).
                _scope_token = None
        ...
        # Bind the profile's COMPLETE terminal policy for the agent build too (fail-closed:
        # malformed policy -> refusal scope) so _make_agent's terminal probing / cwd hints
        # resolve the routed profile, never the launch process (#98581): the unified-desktop
        # reproduction ran a docker-configured profile on the host because terminal_tool read
        # the launch process's pinned env.
...
# Dead-owner claim reclaim throttle (#86721): recover_interrupted_executions opens the executions
# ledger, so the per-tick reap is rate-limited rather than run on every idle 60s cycle. The map is
# profile-local because a multiplex gateway ticks several HERMES_HOME scopes in one process; a
# single scalar lets the first profile suppress recovery for every later profile. Tests may still
# replace this with None / a float to force a reap next tick (backward compatibility).
...
def _maybe_reap_dead_owners() -> None:
    ...
    # Profile-local: a multiplex gateway ticks several HERMES_HOME scopes in one process, and a
    # single scalar would let the first profile suppress recovery for every later profile.
    ...
    if isinstance(_last_dead_owner_reap_at, dict):
        ...
        # Compatibility for tests/extensions that historically reset the scalar
        # to None or an expired timestamp.
        _profile_last_reap_at = _last_dead_owner_reap_at
    if (
        _profile_last_reap_at is not None
        and _reap_now - _profile_last_reap_at < _DEAD_OWNER_REAP_INTERVAL_SECONDS
    ):
        return
    if isinstance(_last_dead_owner_reap_at, dict):
...
def _clear_run_claim_best_effort(job: dict) -> None:
    ...
    Only one-shot jobs carry a ``run_claim`` (stamped by get_due_jobs, #59229), so recurring jobs
    skip the call entirely — clear_run_claim acquires _jobs_lock (blocking cross-process flock) and
    does a full load_jobs read, and the dispatch-failure paths fire exactly when the process can
    least afford pointless lock/read round-trips (interpreter shutdown, EMFILE). clear_run_claim
    itself does load_jobs/save_jobs file I/O; on those degraded paths it can raise, and these
    early-exits exist precisely to skip cleanly — a stale claim expiring at the TTL is a better
    outcome than crashing the tick (#86522). Passing ``expected_owner``/``expected_claim`` keeps
    this narrow: a concurrent runner's replacement claim is never clobbered.
    """
    schedule = job.get("schedule")
...
def _process_due_job(job: dict, adapters, loop, verbose: bool) -> bool:
    ...
    try:
        ...
        # The execution row is provisional until the jobs-store CAS succeeds. A transient
        # jobs.json read/write failure must not leave a live-gateway-owned ``claimed`` row
        # behind: recovery correctly treats that owner as alive, so repeated failures would
        # otherwise accumulate until the process exits.
    ...
    if job.get("execution_fire_claim_pending") is True:
        if isinstance(claimed, dict) and isinstance(claimed.get("_fire_claim_rollback"), dict):
            ...
            # Compatibility for legacy/test overrides that return the historical bool even with
            # return_job=True. Production jobs-store calls return the exact dict and take the
...
def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor, process_job):
    ...
    if not try_register_running_job(job_id):
        ...
        # get_due_jobs stamped a fresh run_claim for THIS scan; the in-flight run holds its own.
        # Release only this scan's exact claim so it does not leak until TTL.
        _clear_run_claim_best_effort(job)
        return None
    ...
    try:
        ...
        dispatched_job = dict(
            job, execution_id=execution["id"],
            execution_fire_claim_pending=(execution.get("fire_claim_acquired") == 0))
    ...
    try:
        ...
        release_running_job(job_id)
        _clear_run_claim_best_effort(job)
        _discard_provisional_best_effort(execution["id"], context="executor submit failed")
...
def tick(
    ...
    try:
        ...
        # Periodic worktree GC (throttled to every 6h, threaded): gateway-only boxes never hit the
        # `hermes -w` startup pruner, so this is the only sweep they get. Same conservative pruner.
        try:
        ...
        # NOTE: no batched ``advance_next_runs`` pre-advance here. The due set is advanced at
        # claim time inside ``claim_job_for_fire``, which records the exact ``_fire_claim_rollback``
        # witness this tick needs to give an occurrence back when the fire never starts. Advancing
        # before the durable claim would consume the occurrence with no witness to roll back.
