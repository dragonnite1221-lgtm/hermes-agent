...
from cron.executions import (
    ...
    discard_unacquired_execution, finish_execution, get_execution, heartbeat_execution,
    mark_execution_handoff_pending, mark_execution_running, mark_fire_claim_acquired,
    recover_interrupted_executions, remember_execution_recovery_intent)
...
def _maybe_reap_dead_owners() -> None:
    ...
    rows whose owner process is proved gone are touched (_owner_is_live). Throttled per profile:
    a multiplex gateway ticks several HERMES_HOME scopes in one process, and a single shared
    scalar would let the first profile's reap suppress recovery for every later profile."""
    ...
    if isinstance(_last_dead_owner_reap_at, dict):
        ...
        # Compatibility for tests/extensions that historically reset the scalar to None or an
        # expired timestamp.
...
def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor, process_job):
    ...
    def _discard_provisional_best_effort(execution_id: str, *, context: str) -> None:
        """The execution row created just above is provisional (fire_claim_acquired=False) until
        the jobs-store CAS succeeds; discard it on a dispatch failure rather than finish_execution
        (which is for an already-acquired attempt)."""
        try:
            discard_unacquired_execution(execution_id)
        except Exception:
            # The process that created this provisional row is still alive, so lease-based
            # recovery alone would ignore it. Preserve local cleanup intent before returning.
            remember_execution_recovery_intent(execution_id)
            logger.exception(
                "Could not discard provisional execution %s after %s; marked for local "
                "reconciliation", execution_id, context)
    ...
    try:
        execution = create_execution(
            job_id, source="builtin", fire_claim_acquired=False,
