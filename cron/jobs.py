...
def create_job(
    ...
    Args:
        ...
        paused: When True, create the job disabled in one write instead of a
                create-then-pause round trip.
        paused_reason: Optional auditable reason; requires ``paused=True``.
    ...
    normalized_script_timeout = _normalize_script_timeout_seconds(
        ...
        has_script=bool(f["script"] or f["monitor_script"]),
    )
...
def _apply_schedule_update(updated: Dict[str, Any], updates: Dict[str, Any], job_id: str) -> None:
    ...
    A pending interrupted-retry marker's owed attempt must not be postponed by a schedule edit:
    the retry keeps firing at its already-persisted ``eligible_at``, and if the marker carries a
    rollback snapshot of the pre-edit ``next_run_at`` it is updated to the newly edited cadence —
    so a later cancellation restores THAT schedule, not the one captured before the edit.
    """
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, i, job):
        ...
        if "retry_interrupted" in updates and not isinstance(updates["retry_interrupted"], bool):
            raise ValueError("retry_interrupted must be a boolean")
        if {"script_timeout_seconds", "script", "monitor_script"}.intersection(updates):
            ...
            if not has_effective_script:
                # Keep the persisted record valid when its last executable source is removed.
                # Otherwise a later script addition can unexpectedly reactivate a stale
                # operator override.
                updates["script_timeout_seconds"] = None
            else:
                timeout_value = updates.get("script_timeout_seconds", job.get("script_timeout_seconds"))
                updates["script_timeout_seconds"] = _normalize_script_timeout_seconds(
                    timeout_value, has_script=True)
        ...
        if {"monitor_script", "monitor_url", "no_agent", "script", "retry_interrupted"}.intersection(updates):
            _upd_script = _normalize_job_optional_text(updated.get("script"))
        ...
        if not schedule_changed and not isinstance(updated.get("schedule"), dict):
            # Match the tick reader's repair contract at the mutation boundary too. A
            # legacy/hand-edited null schedule must not prevent operators from disabling retry
            # or repairing other fields, and the next write should persist a read-safe shape.
            updated["schedule"] = {}
        ...
        if cancel_interrupted_retry:
            ...
            if _job_schedule_kind(updated) == "once" and not schedule_changed:
                ...
                # A simultaneously supplied schedule is a replacement occurrence, not the
                # canceled interrupted attempt. Keep the newly computed one-shot runnable
                # exactly once.
                if updated.get("state") != "paused":
...
def _set_alert_flag(job_id: str, field: str, value: bool) -> bool:
    ...
    survives restarts) and return the PRIOR value. Fields: ``preflight_alerted`` (blocked
...
def _record_run_outcome(
    ...
    if job.get("run_claim") is not None:  # keep key absence for legacy records
        job["run_claim"] = None
    # The owed retry has now actually run (success or failure) — usually already cleared by
    # claim_job_for_fire, but this is the last-resort consumption point for any completion path
    # that reaches mark_job_run without going through the claim (e.g. a direct/legacy caller).
    job.pop("interrupted_retry", None)
    ...
    *,
    ...
    def apply(jobs, _i, job):
        if expected_fire_owner is not None:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_fire_owner:
                logger.warning(
                    ...
                    job_id)
                # Still bump last_run_at: a run genuinely happened just now (this callback firing
                # proves it), even though we can no longer trust it belongs to the CURRENT claim
                # and must not overwrite last_status/failure_streak/fire_claim with a possibly-
                # misattributed outcome. Without this, a gateway restart mid-run (fire_claim
                # re-issued to the new instance before the old instance's completion lands)
                # permanently strands last_run_at at its previous value: the job keeps firing/
                # delivering on schedule but doctor's hermes-cron-missed-daily false-alarms every
                # day after, since nothing else ever advances the stamp. Take the max against the
                # already-persisted value (not an unconditional overwrite): this callback's "now"
                # comes from the STALE owner's clock, which in a multi-machine deployment can lag
                # behind whatever a newer claim owner already recorded — an unconditional
                # assignment could shove last_run_at backward and defeat the whole point of this fix.
                ...
                if existing_last_run_at is None or candidate_last_run_at > existing_last_run_at:
                    job["last_run_at"] = candidate_last_run_at.isoformat()
                    save_jobs(jobs)
                return False
        now = _hermes_now().isoformat()
        consumed_retry = job.get("interrupted_retry")
        ...
        if not success and model_unreachable and not is_terminal_job(job):
            ...
            clear_state(job)
        save_jobs(jobs)
        if isinstance(consumed_retry, dict):
            _forget_interrupted_retry_best_effort(job_id, consumed_retry)
        return True
...
def _claim_job_for_fire_locked(
    ...
    *,
    ...
    leave it false so a stale callback cannot resurrect a paused job.
    ``manual`` marks an off-tick run-now (dashboard trigger) that does NOT
    resume a paused job: unlike ``force``, it must not stamp an occurrence
    identity, since outside a scheduler tick ``next_run_at`` is the NEXT
    occurrence, not the one being run — stamping it would make
    ``completed_occurrence()`` skip that slot when it arrives.
    If a fresh claim (younger than ``claim_ttl_seconds``, and whose owner is
    not provably dead) already exists, lose. If this exact scheduled
    occurrence was already completed, lose (and advance a recurring job's
    stale ``next_run_at``) rather than re-firing it.
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            ...
            job.pop("interrupted_retry", None)
            # This claim is the point after which side effects may exist (cron/occurrences.py):
            # the durable fire_claim above is now the proof of intent to run, so the due-scan's
            # "advanced but never claimed" recovery stamp is no longer needed. Leaving it would
            # make a later due scan "restore" and re-fire this exact occurrence a second time.
            job.pop("pending_slot", None)
            ...
            claimed_job["_scheduled_instant"] = instant
...
def _evaluate_due_job(job: Dict[str, Any], scan: _DueScan, run_claim_ttl: float) -> bool:
    ...
    # The built-in ticker does not take the external fire CAS. Keep a recovered retry out of its
    # due set until the abandoned attempt's terminal ledger handoff has committed. The marker
    # remains until mark_job_run so a crash in the replacement run stays recoverable.
    if not _interrupted_retry_ready(job, now):
        return False
    ...
    # An interrupted-retry's eligible_at is a synthetic, exact recovery timestamp — not a real
    # schedule occurrence. Treat it like a manual run: skip normal-schedule bookkeeping
    # (timezone/cron re-anchoring, fast-forwarding "missed" intervals, occurrence stamping) that
    # would otherwise silently overwrite the owed retry instant before it ever fires.
    is_interrupted_retry = isinstance(job.get("interrupted_retry"), dict)
    bypass_schedule_bookkeeping = manual_run or is_interrupted_retry
    from cron.occurrences import completed_occurrence, scheduled_instant

    if not bypass_schedule_bookkeeping and completed_occurrence(job, next_run):
        new_next = d.recompute_next() if recurring else None
        if new_next:
            scan.persist(job["id"], next_run_at=new_next)
        return False
    if kind == "cron" and not bypass_schedule_bookkeeping and _repair_timezone_shifted_cron(d):
        return False
    ...
    job["_scheduled_instant"] = (
        None if bypass_schedule_bookkeeping else scheduled_instant(job.get("next_run_at"))
    )

    if not bypass_schedule_bookkeeping and kind == "cron" and _reanchor_stale_cron(d):
        return False
    grace = _compute_grace_seconds(d.schedule)
    if not bypass_schedule_bookkeeping and recurring and _fast_forward_missed_recurring(d, grace):
        return False
    if kind == "once":
    ...
    # late catch-up. Recurring only — expired one-shots were retired above; manual triggers and
    # interrupted retries aren't late in the normal-schedule sense.
    if not bypass_schedule_bookkeeping and recurring:
