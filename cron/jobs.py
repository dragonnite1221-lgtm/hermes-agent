...
def create_job(
    ...
    Args:
        ...
        failure_deliver: Optional delivery target used only for failure
                notifications (same value grammar as ``deliver``).
        paused: When True the job is stored paused (the scheduler skips it
                until it is resumed).
        paused_reason: Optional human-readable reason; requires ``paused=True``.
    ...
    # Operator-owned per-job script timeout override (fork feature): only meaningful when the job
    # actually runs a script or monitor_script.
    normalized_script_timeout = _normalize_script_timeout_seconds(
        script_timeout_seconds, has_script=bool(f["script"] or f["monitor_script"]))

    _validate_job_mode_invariants(f["monitor_script"], f["monitor_url"], f["no_agent"], f["script"])
    # Explicit at-least-once restart policy for idempotent no-agent scripts (fork feature).
    normalized_retry_interrupted = _validate_interrupted_retry_mode(
        retry_interrupted, no_agent=f["no_agent"], script=f["script"])
...
def _resolve_job_ref_from_records(
    ...
    """Resolve one ID/name against an already-consistent jobs snapshot. Exact ID wins, then
    case-insensitive name; an ambiguous name raises AmbiguousJobReference rather than silently
    picking one."""
    if not ref:
        return None
    for job in records:
...
def resolve_job_ref(ref: str) -> Optional[Dict[str, Any]]:
    """Resolve an ID or name to a normalized job record. Exact ID match wins (even if a different
    job's name equals this ID), then case-insensitive name; an ambiguous name raises
    AmbiguousJobReference so the caller can surface the matching IDs."""
...
def _normalize_retry_and_timeout_updates(job: Dict[str, Any], updates: Dict[str, Any]) -> None:
    """Normalize the fork's per-job ``retry_interrupted`` / ``script_timeout_seconds`` updates in
    place (invalid values raise BEFORE the merge, like ``_normalize_job_updates``)."""
    if "retry_interrupted" in updates and not isinstance(updates["retry_interrupted"], bool):
        raise ValueError("retry_interrupted must be a boolean")
    if not {"script_timeout_seconds", "script", "monitor_script"}.intersection(updates):
        return
    effective_script = updates.get("script", job.get("script"))
    effective_monitor = updates.get("monitor_script", job.get("monitor_script"))
    if not (effective_script or effective_monitor):
        # Keep the persisted record valid when its last executable source is removed. Otherwise a
        # later script addition can unexpectedly reactivate a stale operator override.
        updates["script_timeout_seconds"] = None
        return
    updates["script_timeout_seconds"] = _normalize_script_timeout_seconds(
        updates.get("script_timeout_seconds", job.get("script_timeout_seconds")), has_script=True)


def _preserve_pending_retry_next_run(updated: Dict[str, Any]) -> None:
    """A schedule edit changes the cadence AFTER an owed interrupted-retry attempt; it must not
    postpone that already-persisted obligation. Runs right after ``_apply_schedule_update``, whose
    freshly computed ``next_run_at`` becomes the marker's rollback snapshot."""
    pending_retry = updated.get("interrupted_retry")
    if not isinstance(pending_retry, dict) or updated.get("state") == "paused":
        return
    updated_next_run = updated.get("next_run_at")
    try:
        retry_eligible_at = _ensure_aware(datetime.fromisoformat(str(pending_retry["eligible_at"])))
    except (KeyError, TypeError, ValueError):
        retry_eligible_at = _hermes_now()
    rollback = pending_retry.get("rollback")
    next_run_snapshot = rollback.get("next_run_at") if isinstance(rollback, dict) else None
    if isinstance(next_run_snapshot, dict):
        # The marker's inverse must follow an operator's schedule edit: if the old owner renews and
        # wins cancellation, restore the edited cadence, not the pre-edit one.
        next_run_snapshot["present"] = updated_next_run is not None
        next_run_snapshot["value"] = copy.deepcopy(updated_next_run)
    updated["next_run_at"] = retry_eligible_at.isoformat()


def _apply_retry_cancellation(updated: Dict[str, Any], *, schedule_changed: bool) -> None:
    """``retry_interrupted=False`` drops an owed attempt: a one-shot with no replacement occurrence
    retires as an explicit unknown-side-effect error, anything else falls back to its schedule."""
    updated.pop("interrupted_retry", None)
    if _job_schedule_kind(updated) == "once" and not schedule_changed:
        repeat = updated.get("repeat")
        if isinstance(repeat, dict):
            times = repeat.get("times")
            completed = repeat.get("completed", 0)
            if isinstance(times, int) and isinstance(completed, int) and completed < times:
        ...
        # A simultaneously supplied schedule is a replacement occurrence, not the canceled
        # interrupted attempt. Keep the newly computed one-shot runnable exactly once.
        if updated.get("state") != "paused":
...
def _forget_cancelled_retry_acks(cancelled_retry: Any, job_id: str) -> None:
    """Best-effort ledger cleanup for a retry marker the operator just canceled."""
    if not isinstance(cancelled_retry, dict):
        return
    try:
        from cron.executions import forget_interrupted_retry_acks

        forget_interrupted_retry_acks(
            _interrupted_retry_execution_ids(cancelled_retry), job_id=job_id)
    except Exception as exc:
        logger.warning(
            "Job '%s': canceled retry acknowledgement cleanup failed: %s", job_id, exc)
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    def apply(jobs, i, job):
        ...
        _normalize_retry_and_timeout_updates(job, updates)
        ...
        if {
            "monitor_script", "monitor_url", "no_agent", "script", "retry_interrupted",
        }.intersection(updates):
            merged_script = _normalize_job_optional_text(updated.get("script"))
        ...
        if not schedule_changed and not isinstance(updated.get("schedule"), dict):
            # Match the tick reader's repair contract at the mutation boundary too: a legacy or
            # hand-edited null schedule must not block disabling retry / repairing other fields.
            updated["schedule"] = {}
        ...
        if schedule_changed:
            ...
            _preserve_pending_retry_next_run(updated)
        if cancel_interrupted_retry:
            _apply_retry_cancellation(updated, schedule_changed=schedule_changed)
        ...
        jobs[i] = updated
        save_jobs(jobs)
        _forget_cancelled_retry_acks(cancelled_retry, job_id)
...
def resume_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    with _jobs_lock():
        ...
        if is_terminal_job(job) and not _is_recoverable_error_job(job):
            ...
            raise ValueError(
                f"Cannot resume: job '{name}' is {state} (terminal). "
...
def remove_job(job_id: str) -> bool:
    ...
    with _jobs_lock():
        ...
        # A removed job's owed interrupted-retry attempt dies with it: drop its ledger acks so they
        # cannot outlive the record (best effort — never block the removal).
        _forget_cancelled_retry_acks(removed_retry, canonical_id)
        return True
...
def mark_drift_alerted(job_id: str) -> bool:
    """Mark the job as drift-alerted (#44585 drift-guard skip); True if it already was.
    (The matching clear lives in the plugin-compat block at the end of this module.)"""
...
def _record_run_outcome(
    ...
    if success:
        ...
        # the forward-failure stamp so it only describes CURRENT auto-fire health. A run that made
        # it through the guard also means resolution matches again (drift marker, #44585).
        ...
        if expected_fire_owner is not None:
            ...
            if not isinstance(claim, dict) or claim.get("by") != expected_fire_owner:
                logger.warning(
                    ...
                    job_id)
                return False
        # A durable terminal result retires the owed interrupted-retry attempt: this run IS the
        # retry (or supersedes it), so the marker is consumed before the record is rewritten.
...
def clear_run_claim(
    ...
    *,
    ...
    ``expected_owner`` / ``expected_claim`` make the clear a compare-and-swap: a claim another
    process has since taken over (or refreshed) is left alone.
    ...
    """
    def apply(jobs, _i, job):
        if _job_schedule_kind(job) != "once" or job.get("run_claim") is None:
            return False  # recurring, or already cleared
        claim = job.get("run_claim")
        if expected_claim is not None and claim != expected_claim:
            return False  # another process re-claimed or refreshed it
        if isinstance(claim, dict) and expected_owner is not None \
    ...
    with _jobs_lock():
        ...
        for job in jobs:
            if (
                ...
                or _job_schedule_kind(job) not in {"cron", "interval"}
            ):
                continue
...
def _machine_id() -> str:
    ...
    Deliberately delegates to the execution ledger's machine identity instead of upstream's
    ``HERMES_MACHINE_ID`` env escape hatch: claim ownership is behavioral data for the
    interrupted-retry liveness fence, and both stores must agree on what "this host" means.
    """
...
def claim_job_for_fire(
    ...
    manual: bool = False, return_job: bool = False, execution_id: Optional[str] = None,
    ...
    def apply(jobs, _i, job):
        ...
        if not _interrupted_retry_ready(job, now):
            # An owed attempt whose jobs.json -> ledger handoff is not proven terminal stays owed.
            return False
        ...
        # stamping it would make completed_occurrence() skip that slot when it arrives. An owed
        # interrupted retry has the same shape — its ``next_run_at`` is the retry's eligibility
        # instant, not a schedule occurrence.
        manual_fire = (
            force or manual or isinstance(interrupted_retry, dict)
        ...
        if instant and completed_occurrence(job, instant):
            if _job_schedule_kind(job) in {"cron", "interval"}:
                ...
                if nxt:
                    ...
                    save_jobs(jobs)
            return False
        # Keep an in-memory undo record for the second half of the cross-store hand-off. The
        # jobs-store CAS necessarily commits before the execution-ledger fence; if that promotion
        # cannot be committed, the caller uses this exact snapshot to put the occurrence back
        # instead of silently consuming it.
        due_run_claim = copy.deepcopy(job.get("run_claim"))
        rollback = {
            key: {"present": key in job, "value": copy.deepcopy(job.get(key))}
            for key in (
                "next_run_at", "interrupted_retry", "repeat", "enabled", "state", "paused_at",
                "paused_reason",
            )
        }
        if force:
        ...
        if not manual and _job_schedule_kind(job) in {"cron", "interval"}:
            ...
            if nxt:
                job["next_run_at"] = nxt
        # Record the exact values produced by this claim. Rollback is a three-way merge (before /
        # claimed / current), not an overwrite: a concurrent operator edit landing after the
        # jobs-store CAS must stay authoritative when the execution fence later fails.
        ...
        if "interrupted_retry" in rollback:
            # Restoring an owed retry is valid only while the explicit at-least-once policy is
            # still enabled. This guard resolves the otherwise-ambiguous absent->absent state when
            # an operator disables the policy after the claim consumed the marker but before
            # ledger promotion fails.
            rollback["_retry_policy_claimed"] = copy.deepcopy(job.get("retry_interrupted"))
        if isinstance(due_run_claim, dict):
            # get_due_jobs() claims a finite one-shot before ledger/fire setup begins. A clean
            # pre-run abort must release that exact due-scan owner too; otherwise the restored
            # occurrence stays hidden until the much longer run-claim TTL expires.
            rollback["_run_claim_to_clear"] = due_run_claim
        save_jobs(jobs)
        if not return_job:
            # Legacy bool callers have no second phase, so they keep the historical cleanup.
            _forget_interrupted_retry_best_effort(job_id, interrupted_retry)
            return True
        claimed_job = dict(copy.deepcopy(job), _scheduled_instant=instant)
        claimed_job["_fire_claim_rollback"] = rollback
        if not execution_id:
            # Ledger-first callers finalize the acknowledgement only after their execution row is
            # fenced as the CAS winner (see finalize_fire_claim_setup).
            _forget_interrupted_retry_best_effort(job_id, interrupted_retry)
        return claimed_job
...
def heartbeat_fire_claim(job_id: str, *, expected_owner: str) -> bool:
    """Refresh an active ``fire_claim`` without extending another owner's lease: a cron execution
    can legitimately outlive the TTL, and comparing the owner copied at dispatch stops a stale
    runner from refreshing a claim another process has since recovered.

    Deliberately NOT taken under ``_fire_job_lock``: the jobs-store owner CAS is the authoritative
    fence, and keeping renewal independent of that per-job lock lets a long network side effect
    retain its lease while recovery waits on the same fence.
    """
...
def _fast_forward_missed_recurring(d: _DueJob, grace: int) -> None:
    ...
    if (d.scan.now - d.next_run_dt).total_seconds() <= grace:
        return
    if isinstance(d.job.get("interrupted_retry"), dict):
        # An owed interrupted-retry attempt is not a missed cadence slot: its ``next_run_at`` is the
        # retry's eligibility instant, so fast-forwarding past it would silently drop the attempt.
        return
    new_next = d.recompute_next()
    if not new_next:
        return
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        try:
            ...
            if not job.get("enabled", True):
                continue
            # The built-in ticker does not take the external fire CAS. Keep a recovered retry out
            # of its due set until the abandoned attempt's terminal ledger handoff has committed.
            # The marker remains until mark_job_run so a crash in the replacement run stays
            # recoverable.
            if not _interrupted_retry_ready(job, scan.now):
                continue
